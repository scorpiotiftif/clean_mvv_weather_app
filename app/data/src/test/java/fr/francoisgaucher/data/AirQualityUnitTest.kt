...
import fr.francoisgaucher.data.RessourceHelper.Companion.getFileInputStreamFromJsonAsset
import fr.francoisgaucher.data.RessourceHelper.Companion.readContentFromJsonAsset
...
class AirQualityUnitTest {
    ...
    @Test
    fun asset_foundable() {
        val air_quality_result =
            getFileInputStreamFromJsonAsset("meteo_air_quality_latest_latlng_200.json")
    ...
    fun `test that the convertion is OK from JSON response AND convert AirQualityData TO AirQuality`() {
        val air_quality: String? =
            readContentFromJsonAsset("meteo_air_quality_latest_latlng_200.json")
        println(air_quality)
